...
onlysources.tex	        ?= $(shell grep -l "[\]begin{document}" *.tex | xargs echo)
#onlysources.lhs	?=
#onlysources.tex.sh	?=
#onlysources.tex.pl	?=
#onlysources.tex.py	?=
...
move-files-helper	= $(if $1,$(MV) $1 $2 2>/dev/null || true,$(sh_true))
...
move-files		= \
	$(call move-files-helper,$(call moveable-files,$(wildcard $1)),$2)
...
default_stems_ss	:= $(sort $(default_stems_source))
...
#
#	#$(QUIET) \
#	#if [ -d $(TRASH_DIR) -a "$(TRASH_DIR)" != "." ]; then \
#		#$(call move-files, \
#			#$(foreach e,$(addsuffix .fls,$(all_stems_source)), \
#			#$(shell $(call get-generated-names,$e))), \
#			#$(TRASH_DIR) \
#		#); \
#		#$(call move-files, \
#			#$(all_d_targets) *.make *.make.temp *.cookie, \
#			#$(TRASH_DIR) \
#		#); \
#		#$(call move-files,$(graph_stem).*,$(TRASH_DIR)); \
#		#$(call move-files,$(rm_tex),$(TRASH_DIR)); \
#	#fi; 
#
.PHONY: all
...
TO_MOVE      = *.aux *.log *.toc *.lof *.lot *.bbl *.blg *.out *.nav *snm \
               *.vrb *.rel *.make *.nlg *.nlo *.nls *.fls *.d
...
move_to_trash:
	@if [ ! -z "${TO_TRASH}" -a -d $(TRASH_DIR) -a "$(TRASH_DIR)" != "." ]; then  \
                echo "==> Now moving Files to $(TRASH_DIR)/";                   \
                mv -f ${TO_TRASH} $(TRASH_DIR)/ 2>/dev/null;                          \
