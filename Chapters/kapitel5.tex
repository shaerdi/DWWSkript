...
\begin{tikzpicture}
    ...
    \node[below] at (omegaR0) {$\omega_0$};
    \node[below] at (omegaR1) {$\underset{\delta=1}{\omega_r}$};
    \node[below] at (omegaR2) {$\underset{\delta=2}{\omega_r}$};
    
    \draw[dotted] (omegaR0) -- ++ (axis cs:0, 0.2);
    \draw[dotted] (omegaR1) -- ++ (axis cs:0, {A(wr(1),1});
    \draw[dotted] (omegaR2) -- ++ (axis cs:0, {A(wr(2),2});
...
\begin{tikzpicture}
    \begin{axis}[
        ...
        \pgfmathdeclarefunction{phiF}{2}{%
            \pgfmathparse{%
                rad(atan2(2*\DELTA*#1, #2^2-#1^2))
            }%
        }
        \addplot[mark=none,samples=100,domain=-1:1] (
            {x},
            {phiF(x,1)}
        );
    \end{axis}
