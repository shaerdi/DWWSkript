...
\begin{eqnarr}
    ...
    \sqrt{
        \vphantom{\omega^2}
        \smash{\underbrace{\delta^2-\omega_0^2}_{-\omega^2}}
    }\\
\end{eqnarr}
\begin{equation*}
...
\begin{eqnarr}
    ...
    \text{Re}\left[ \tilde{y}_1(x) \right] &=& e^{-\delta x}\cdot \cos(\omega x) \\
    \text{Im}\left[ \tilde{y}_1(x) \right] &=& e^{-\delta x}\cdot \sin(\omega x) \\
\end{eqnarr}
...
\begin{eqnarr}
    \omega &=& \sqrt{\omega_0^2-\delta^2} = \sqrt{1-0} = 1\\
