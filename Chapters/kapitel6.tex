...
\end{center}
\begin{eqnarr}
    \L \left( \sigma(t) \right) &=& \int_0^\infty e^{-st}\dd{t} \\
...
\end{center}
\begin{eqnarr}
    \L \left( f(t) \right) &=& \int_0^\infty te^{-st}\dd{t} \\
...
\end{center}
\begin{eqnarr}
    \L \left( f(t) \right) &=& \int_0^\infty \cos(\alpha t)e^{-st}\dd{t} \\
    &\overset{\tiny{(324)}}{=}&  \left.   
...
\underline{Beweis}
\begin{eqnarr}
    \L\left[ f(at) \right] &=& \int_0^\infty f(at) e^{-st}\dd{t} \\
    &=& \int_0^\infty f(u) e^{-\frac{s}{a}u}\frac{\text{d}u}{a} 
    \hspace{2em} \text{mit } u=at,~\diff{u}{t}=a\\
...
\begin{eqnarr}
    \L \left[ f(t-a) \right] 
    &=&  \int_0^\infty f(t-a)e^{-st}\dd{t}\\
    &=&  \int_a^\infty f(t-a)e^{-st}\dd{t} \hspace{2em} u=t-a\\
    &=& \int_0^\infty f(u) e^{-s(u+a)}\dd{u}\\
    &=& \int_0^\infty f(u) e^{-su}e^{-sa}\dd{u}\\
    &=& e^{-as}F(s)
\end{eqnarr}
...
\begin{equation*}
    \boxed{
        \L\left[ t\cdot f(t) \right] = -\diff{}{s}F(s),
        \hspace{1em} \text{wobei } f(t)\multimapdotbothA F(s)
    }
\end{equation*}
\underline{Beweis}
\begin{eqnarr}
    \L\left[ t\cdot f(t) \right] &=& 
        \int_0^\infty tf(t) e^{-st}\dd{t}\\
        &=& \int_0^\infty f(t) \left(   -\diff{}{s}e^{-st}
        \right)\dd{t}\\
        &=& -\diff{}{s}\int_0^\infty f(t)  e^{-st} \dd{t}\\
        &=& -\diff{}{s}F(s)\\
\end{eqnarr}

\bsp{Beispiel}
\begin{eqnarr}
    e^{3t}&\multimapdotbothA & \frac{1}{s-3}\\
    &\Rightarrow&\\
    t\cdot e^{3t}&\multimapdotbothA& -\frac{\text{d}}{\text{d}s}\frac{1}{s-3}\\
...
\begin{eqnarr}
    ...
    \int_0^\infty e^{-at} f(t) e^{-st}\dd{t}\\
    &=& \int_0^\infty f(t) e^{-(s+a)t}\dd{t}\\
    &=& F(s+a)
\end{eqnarr}
...
$\Rightarrow$
\begin{eqnarr}
    \L\left[ t^2 \right] &=& \L\left[ \int_0^t 2u\dd{u} \right]\\
...
\begin{eqnarr}
    ...
    \underbrace{e^{-st}}_{v} \dd{t} \\
    &=& -\int_0^\infty f(t)(-s)e^{-st}\dd{t}+\left.f(t)\right|_0^\infty\\
    &=& s\int_0^\infty f(t)e^{-st}\dd{t}-f(0)\\
    &=& sF(s)-f(0)\\
\end{eqnarr}
...
\bsp{Beispiel}
\begin{eqnarr}
    \cos(0) &=& \lim_{s\rightarrow\infty}s\L\left[ \cos(t) \right]\\
     &=& \lim_{s\rightarrow\infty}s\frac{s}{s^2+1} \\
     &=& \lim_{s\rightarrow\infty}
                \frac{s^2}{s^2\left( 1+\frac{1}{s^2} \right)} \\
     &=& \lim_{s\rightarrow\infty} \frac{1}{1+\frac{1}{s^2}} \\
     &=& 1
\end{eqnarr}
...
%\begin{center}
    %\begin{tikzpicture}
        %\draw[->] (0,0) -- ++(1,0) node[midway,above] {$u(t)$};
        %\draw[->] (1,-0.5) rectangle ++(2,1) ;
        %\draw[->] (3,0) -- ++(1,0) node[midway,above] {$x(t)$};
    %\end{tikzpicture}
%\end{center}
%System im Laplace-Bereich:
%\begin{center}
    %\begin{tikzpicture}
        %\draw[->] (0,0) -- ++(1,0) node[midway,above] {$U(s)$};
        %\draw[->] (1,-0.5) rectangle ++(2,1) node[midway] {$G(s)$} ;
        %\draw[->] (3,0) -- ++(1,0) node[midway,above] {$X(s)$};
        %\node at (2,-1.5) {$X(s)=G(s)\cdot U(s)$};
    %\end{tikzpicture}
%\end{center}
...
Entsprechend ist auch die Terminologie anders:
\begin{outline}
...
\begin{center} \begin{tikzpicture} 
    \draw[thick, ->] (0,0) -- (2,0) node[midway, above] {$u_{\text{in}}$};
    \draw[thick] (2,-1) rectangle ++(4,2) node[midway] {System};
    \draw[thick, ->] (6,0) -- ++(2,0) node[midway, above] {$u_{\text{out}}$};
...
\begin{eqnarr}
    RC(sU-u(0)) + U &=& \L\left[ u_{\text{in}} \right] \\
    &\Rightarrow& \\
    U = \underbrace{\frac{1}{RCs+1}}_{G(s)}
    \cdot \L\left[ u_{\text{in}} \right] \\
\end{eqnarr}
\begin{outline}
...
\begin{center}
    \begin{tikzpicture}
        \draw[->] (0,-1) -- (0,2);
        \draw[->] (-4,0) -- (4,0);
        \draw[ultra thick] (-4,0) -- (0,0) -- (0,1) -- (4,1);
    \end{tikzpicture}
\end{center}

Die Antwort auf die Sprungfunktion heisst \underline{Sprungantwort}.

\bsp{Beispiel:} S5 A17b

Impulsfunktion $\delta(t)$:
\begin{center}
    \begin{tikzpicture}
        \draw (0,-1) -- (0,0);
        \draw[ultra thick] (-4,0) -- (0,0) -- (0,2) + (0,-2) -- (4,0);
    \end{tikzpicture}
\end{center}
